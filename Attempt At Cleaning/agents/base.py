...
class BaseAgent:
    ...
    def extra_deposit_metadata(self) -> dict:
        """Extra metadata keys merged into each deposit. Override in subclasses.

        Called once per deposit attempt (after sample() and LLM generation).
        Keys returned here are merged BEFORE proposed_type / proposed_parent
        so those always win on conflict. Subclasses store per-iteration state
        (e.g. targets_cluster_id in Hater) during sample() and read it here.
        """
        return {}
