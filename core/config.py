...
ROLE_TO_ENGINE = {
    ...
    "research_ensemble": {
        # List-form roles bind STABLY per worker (MultiEngineRouter.engine_for
        # with worker_id): each scout keeps one family for the whole run, so
        # each corpus partition is explored by one consistent prior and
        # different partitions by different priors. The offsets between the
        # scout/developer/validator lists mean a claim generated by family A
        # tends to be developed and verified by families B/C — CROSS-FAMILY
        # verification. Same-family verification is epistemically weak: the
        # prior that generated a claim will also tend to confirm it.
        "scout":       ["primary", "secondary", "tertiary"],
        "forager":     ["secondary", "tertiary", "primary"],
        "developer":   ["secondary", "tertiary", "primary"],
        "critic":      "secondary",
        "hater":       "tertiary",
        "validator":   ["tertiary", "secondary"],
        "synthesizer": "primary",
    },
    "small": {
        "scout":       "primary",
